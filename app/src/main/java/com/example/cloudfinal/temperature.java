...
public class temperature extends AppCompatActivity {
    ...
    // the result from db
    PaginatedList<HackTempDO> dbresult;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        Button btn_tempup = findViewById(R.id.btn_tempup);
        ...
        Button btn_tempdown = findViewById(R.id.btn_tempdown);
        btn_tempdown.setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View view) {
                callCloudLogic();
            }
    ...
    Thread query= new Thread(new Runnable() {
        @Override
        public void run() {
            HackTempDO hackTempDO = new HackTempDO();
            hackTempDO.setPrimary("p");
            hackTempDO.setTimeStamp("2019-01");
            ...
            DynamoDBQueryExpression queryExpression = new DynamoDBQueryExpression()
                    .withHashKeyValues(hackTempDO)
                    .withRangeKeyCondition("timeStamp", rangeKeyCondition)
                    .withConsistentRead(false);

            dbresult = dynamoDBMapper.query(HackTempDO.class, queryExpression);
    ...
    public void handlejson(PaginatedList<HackTempDO> paginatedList){
        ...
        for (int i = 0; i < paginatedList.size(); i++) {
            ...
            try {
                ...
                tmp_array.add(ID);
                tmp_array.add(temperature);
                tmp_array.add(timeStamp);

                rowlist.add(tmp_array);

                //Log.e("My App", obj.toString());
                Log.e("attribute:", "ID is " + ID +", temperature is " + temperature + ", timeStamp is " + timeStamp);
    ...
    public void createTable(){
        // set table
        tab = findViewById(R.id.table_temp);
...
/*
    public void createUser() {
        final TestDbDO incomeItem = new TestDbDO();

        incomeItem.setUserId("even");

        incomeItem.setIncome(1000000.0);
        incomeItem.setHouse(3.0);
    ...
    public void readUser() {
        new Thread(new Runnable() {
            ...
            public void run() {
                ...
                TestDbDO incomeItem = dynamoDBMapper.load(
                        TestDbDO.class,
                        "even",
                        1000000.0);

                // Item read
                Log.e("Item user", incomeItem.getUserId());
                Log.e("Item house", Double.toString(incomeItem.getHouse()));
                Log.e("Item income", Double.toString(incomeItem.getIncome()));
            }
        }).start();
    }
    ...
    public void deleteNews() {
        new Thread(new Runnable() {
            ...
            public void run() {
                ...
                TestDbDO newsItem = new TestDbDO();

                newsItem.setUserId("even");    //partition key

                newsItem.setIncome(1000000.0);  //range (sort) key
